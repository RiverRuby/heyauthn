...
function c(e) {
  return btoa(B(e)).replaceAll("+", "-").replaceAll("/", "_")
}
function s(e) {
...
async function v(e, t, a) {
  ...
  let n = {
    ...
    user: { id: await u(new TextEncoder().encode(e)), name: e, displayName: e },
    pubKeyCredParams: [
    ...
    authenticatorSelection: {
      userVerification: a.userVerification ?? "required",
      authenticatorAttachment: await R(a.authenticatorType ?? "auto"),
    },
    attestation: "direct",
  }
  a.debug && console.debug(n)
  let r = await navigator.credentials.create({ publicKey: n })
  a.debug && console.debug(r)
  let i = r.response,
    o = {
      username: e,
      credential: {
        id: r.id,
        publicKey: c(i.getPublicKey()),
        algorithm: V(r.response.getPublicKeyAlgorithm()),
      },
      authenticatorData: c(i.getAuthenticatorData()),
      clientData: c(i.clientDataJSON),
    }
  return a.attestation && (o.attestationData = c(i.attestationObject)), o
}
async function k(e) {
  let t = ["internal"],
    a = ["hybrid", "usb", "ble", "nfc"]
  if (e === "local") return t
  if (e == "roaming" || e === "extern") return a
  if (e === "both") return [...t, ...a]
  try {
    return (await A()) ? t : a
  } catch {
    return [...t, ...a]
...
async function Y(e, t, a) {
  if (((a = a ?? {}), !g(t)))
    throw new Error("Provided challenge is not properly encoded in Base64url")
  let n = await k(a.authenticatorType ?? "auto"),
    r = {
      challenge: s(t),
      rpId: window.location.hostname,
      allowCredentials: e.map((d) => ({
        id: s(d),
        type: "public-key",
        transports: n,
      })),
  ...
  a.debug && console.debug(r)
  let i = await navigator.credentials.get({ publicKey: r })
  a.debug && console.debug(i)
  let o = i.response
  return {
    credentialId: i.id,
    authenticatorData: c(o.authenticatorData),
    clientData: c(o.clientDataJSON),
    signature: c(o.signature),
  }
}
var U = {}
f(U, {
  verifyAuthentication: () => q,
  verifyRegistration: () => j,
  verifySignature: () => x,
})
var K = {}
f(K, {
...
function w(e) {
  return typeof e == "string" && (e = s(e)), JSON.parse(z.decode(e))
}
function P(e) {
  return typeof e == "string" && (e = s(e)), O(e)
}
function F(e) {
  return (
...
function C(e) {
  return {
    ...
    client: w(e.clientData),
    authenticator: P(e.authenticatorData),
    attestation: e.attestationData ? F(e.attestationData) : null,
  }
}
function S(e) {
  return {
...
async function j(e, t) {
  let a = C(e)
  if ((a.client.challenge, a.client.type !== "webauthn.create"))
    throw new Error(`Unexpected ClientData type: ${a.client.type}`)
  if (await m(t.origin, a.client.origin))
    throw new Error(`Unexpected ClientData origin: ${a.client.origin}`)
  if (await m(t.challenge, a.client.challenge))
    throw new Error(`Unexpected ClientData challenge: ${a.client.challenge}`)
  return a
}
async function q(e, t, a) {
  if (e.credentialId !== t.id)
  ...
  if (await m(a.origin, r.client.origin))
    throw new Error(`Unexpected ClientData origin: ${r.client.origin}`)
  if (await m(a.challenge, r.client.challenge))
  ...
  if (!r.authenticator.flags.userVerified && a.userVerified)
    throw new Error("Unexpected authenticator flags: missing userVerified")
  if (r.authenticator.counter <= a.counter)
    throw new Error(
      `Unexpected authenticator counter: ${r.authenticator.counter} (should be > ${a.counter})`
    )
  return r
}
function G(e) {
...
async function Q(e, t) {
  let a = s(t)
  return crypto.subtle.importKey("spki", a, e, !1, ["verify"])
}
async function x({
  algorithm: e,
  publicKey: t,
  ...
  console.debug("Crypto Algo: " + JSON.stringify(i)),
    console.debug("Public key: " + t),
    console.debug("Data: " + c(d)),
    console.debug("Signature: " + r)
  let p = s(r)
  return e == "ES256" && (p = Z(p)), await crypto.subtle.verify(i, o, p, d)
}
function Z(e) {
...
export { I as client, K as parsers, U as server, l as utils }
